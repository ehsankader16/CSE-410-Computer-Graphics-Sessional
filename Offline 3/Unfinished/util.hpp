...
bitmap_image blackTexture = bitmap_image("texture_b.bmp");
bitmap_image whiteTexture = bitmap_image("texture_w.bmp");
    ...
    double intersect(Ray& ray, Color& pixelColor, int currentLevel, int recursionLevel) {
        ...
        // if(currentLevel < recursionLevel) {
            ...
            if(nearestObjectIndex != -1) {
                ...
                double t = objects[nearestObjectIndex]->intersect(reflectedRay, tempColor, currentLevel + 1, recursionLevel - 1);
...
class CheckerBoard : public Object
{
    ...
    Color getColorAtPoint(Vector point) {
        ...
        double tileX = (point.x - referencePoint.x) - i * tileWidth;
        double tileY = (point.y - referencePoint.y) - j * tileWidth;
        
        int xPixel = (int)abs((whiteTexture.width() * tileX) / tileWidth);
        int yPixel = (int)abs((whiteTexture.height() * tileY) / tileWidth);
        cout<<" width "<<whiteTexture.width()<<" height "<<whiteTexture.height()<<endl;
        cout <<" xPixel "<<xPixel<<" yPixel "<<yPixel<<endl;
        if((i + j) & 1) {
            if (textureMode) {
                // rgb_t textureColor;
                // blackTexture.get_pixel(xPixel, yPixel, textureColor);
                // Color color;
                // color.r = (int)textureColor.red;
                // color.g = (int)textureColor.green;
                // color.b = (int)textureColor.blue;
                // return color;
                return Color(0, 0, 0);
            } else {
                return Color(0, 0, 0);
            ...
            if (textureMode) {
                // rgb_t textureColor;
                // whiteTexture.get_pixel(xPixel, yPixel, textureColor);
                // Color color;
                // color.r = (int)textureColor.red;
                // color.g = (int)textureColor.green;
                // color.b = (int)textureColor.blue;
